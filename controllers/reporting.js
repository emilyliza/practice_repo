module.exports = function(app) {
	...
	app.get('/api/v1/report/customers?', mw.auth(), function(req, res, next) {
		...
		.seq(function(data) {
			...
			var result1 = [],
				result2 = [];
			_.each(data, function(row) {
				result1.push( { 'name': row._id.name, "sum": row.sum, "count": row.count } );
				result2.push( { 'name': row._id.name, "sum": row.sum, "count": row.count } );
			});
	
		return res.json({
				"byVolume": {
					"label": 'Customers By Volume',
					"data_type": 'currency',
					"filter": {
						'name': '$_id.name'
					},
	...
	app.get('/api/v1/report/midCodes?', mw.auth(), function(req, res, next) {
		...
		var search = [
				...
				{ '$group': { 
					"_id": {
						'mid': '$portal_data.MidNumber', 'reasonCode': '$portal_data.ReasonCode'
					},
					"count": { '$sum': 1 },
					'sum': { '$sum': '$portal_data.ChargebackAmt'}
				}},
	...
	app.get('/api/v1/report/parentCodes?', mw.auth(), function(req, res, next) {
		...
		var search = [
				...
				{ '$group': { 
					"_id": {
						'user': '$user', 'reasonCode': '$portal_data.ReasonCode'
					},
					"count": { '$sum': 1 },
					'sum': { '$sum': '$portal_data.ChargebackAmt'}
				}},
	...
	app.get('/api/v1/report/midCustomers?', mw.auth(), function(req, res, next) {
		...
		var search = [
				...
				{ '$group': { 
					"_id": {
						'mid': '$portal_data.MidNumber', 'fullName': '$gateway_data.FullName'
					},
					"count": { '$sum': 1 },
					'sum': { '$sum': '$portal_data.ChargebackAmt'}
				}},
	...
	app.get('/api/v1/report/parentCustomers?', mw.auth(), function(req, res, next) {
		...
		var search = [
				...
				{ '$group': { 
					"_id": {
						'user': '$user', 'fullName': '$gateway_data.FullName'
					},
					"count": { '$sum': 1 },
					'sum': { '$sum': '$portal_data.ChargebackAmt'}
				}},
